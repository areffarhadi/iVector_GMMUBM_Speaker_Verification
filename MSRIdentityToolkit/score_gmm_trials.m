...
% end
parfor testIdx = 1 : tlen
    fea = htkread(testFiles{testIdx});
    ubm_llk = compute_llk(fea, ubm.mu, ubm.sigma, ubm.w(:));
    for i = 1 : ntrials
        if (trials(i, 2) == testIdx)
            gmm = models{trials(i, 1)};
            gmm_llk = compute_llk(fea, gmm.mu, gmm.sigma, gmm.w(:));
            llr(testIdx, i) = mean(gmm_llk - ubm_llk);
            %fprintf('%d\t%d\t%f\n', testIdx, i, mean(gmm_llk - ubm_llk));
        end
    end
    fprintf('Finished Test of file : %d\n', testIdx);
end
llr = sum(llr)';
